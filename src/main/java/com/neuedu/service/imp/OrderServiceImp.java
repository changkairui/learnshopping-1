...
public class OrderServiceImp implements IOrderService {
    ...
    public ServerResponse alipay_callback(Map<String, String> map) {
        ...
        Order order =  orderMapper.finfOrderByOrderNo(orderNo);
        if (order==null){
            return ServerResponse.createServerResponseByErrow("订单"+orderNo+"不是本商品的订单");
        }

        if (order.getStatus()>=Const.OrderStatusEnum.ORDER_PAYED.getCode()){
            //防止支付宝重复回调
            return ServerResponse.createServerResponseByErrow("支付宝重复调用");
        }
        ...
        return ServerResponse.createServerResponseByErrow("支付信息保存失败");
    ...
    public ServerResponse query_order_pay_status(Long orderNo) {
        if (orderNo==null){
            return ServerResponse.createServerResponseByErrow("订单号不能为空");
        }
        Order order = orderMapper.finfOrderByOrderNo(orderNo);
        if (order==null){
            return ServerResponse.createServerResponseByErrow("订单不存在");
        }
        if (order.getStatus()==Const.OrderStatusEnum.ORDER_PAYED.getCode()){
    ...
    public ServerResponse pay(Integer userId, Long orderNo) {
        ...
        try {
            ...
            return ServerResponse.createServerResponseBySucess("cg",serverResponse);
        } catch (Exception e) {
            e.printStackTrace();
        }

        return ServerResponse.createServerResponseByErrow("下单失败");
    }
...
//                + (long) (Math.random() * 10000000L);
        ...
        AlipayTradePrecreateRequestBuilder builder = new AlipayTradePrecreateRequestBuilder()
                ...
                //内网穿透
                .setNotifyUrl("http://h4mbtp.natappfree.cc/portal/order/alipay_callback.do")//支付宝服务器主动通知商户服务器里指定的页面http路径,根据需要设置
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                //"/Users/sudo/Desktop/qr-%s.png    $是个占位符，二维码名字是订单号"
                String filePath = String.format("D:\\facetest\\ftpfile/qr-%s.png",
